...
[h w nc] = size(I_in); % height width numberofcolors
configs.h = h;
configs.w = w;
configs.num_px = h*w; % number of pixels
...
for i = 1 : niter
  ...
  opts.printEvery     = 50;
  l = zeros(numel(z),1); % lower bounds
  u = ones(numel(z),1); % upper bounds
...
function out = merge_two_patches(est_t, est_r, h, w, psize)
  ...
  out = cat(1, t_merge(:), r_merge(:)); % concatenate along first dimension
