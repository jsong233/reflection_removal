...
for i = 1:ch
    ...
    for j = 1:MAX_ITER
        ...
        WT = @(x) iswt2(Z.*x(:,:,1:level_response), wavelet_order, wavelet_level) + gamma*x(:,:,1+level_response:end).*T/norm(T,'fro');
        r = SplitBregAnalysis(g, A, AT, W, WT, opts1); R = r;
    
        l = Y - A(R);opts2.u0 = T; 
        M = @(x) cat(3,Z1.*swt2(x, wavelet_order, wavelet_level),gamma*imageLike(R,x)*ones(h,w));
        MT = @(x) iswt2(Z1.*x(:,:,1:level_response), wavelet_order, wavelet_level) + gamma*x(:,:,1+level_response:end).*R/norm(R,'fro');
